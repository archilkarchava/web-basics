...
class Register extends Component {
    ...
    handleSubmit(e) {
        e.preventDefault();
        if (this.state.username.length > 0 && this.passwordRegex.test(this.state.password) && this.emailRegex.test(this.state.email) && this.phoneRegex.test(this.state.phone)) {
            var payload = {
            ...
            alert("Телефон должен начинаться с +7 или 8 и состять из 11 цифр. Допускается ввод через дефисы!")
        }
    }
    render() {
        return (
